...
class QuestionBank:
    ...
    def get_random(self, subject=None):
        pool = [q for q in self.questions if subject is None or q["subject"].lower()==subject.lower()]
...
class TheMeowGames:
  ...
  def play_quiz(self, count=10):
    player = self.select_or_create_player()
    pause(f"Quiz Mode: {count} questions. Press Enter to start...")
    asked = set()
